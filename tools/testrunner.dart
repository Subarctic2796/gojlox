...
bool _runSuite(String name) {
  ...
  if (_failed == 0) {
    print("All ${green(_passed)} tests passed "
        "($_expectations expectations).");
  } else {
    print("${green(_passed)} tests passed. "
        "${red(_failed)} tests failed.");
  }
...
void _runTest(String path) {
  ...
  var grayPath = gray("($path)");
  writeLine("Passed: ${green(_passed)} "
      "Failed: ${red(_failed)} "
      "Skipped: ${yellow(_skipped)} $grayPath");
...
class Test {
  ...
  bool parse() {
    ...
    if (_expectedErrors.isNotEmpty && _expectedRuntimeError != null) {
      print("${magenta('TEST ERROR')} $_path");
